...
def health_ready() -> JSONResponse:
    """503 until pipeline and BM25 index are available (for orchestrators / K8s readinessProbe)."""
...
@app.post("/rag/chat")
def rag_chat(request: RagChatRequest) -> dict[str, Any]:
    if pipeline is None:
...
@app.post("/rag/chat/simple")
def rag_chat_simple(request: RagChatSimpleRequest) -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
    ...
    target_province=request.targetProvince,
    ...
    return {
        ...
        "rag_mode": result.get("rag_mode", request.mode),
...
@app.post("/admin/ai/debug-query")
def admin_ai_debug_query(request: AdminAiDebugQueryRequest) -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
@app.post("/admin/rag/retrieve-debug")
def admin_rag_retrieve_debug(request: AdminRagRetrieveDebugRequest) -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
@app.get("/admin/rag/place/{place_id}")
def admin_rag_place_detail(place_id: str) -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
@app.post("/admin/rag/place-store/reload")
def admin_rag_place_store_reload() -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
@app.get("/admin/rag/places/search")
...
def admin_cache_status() -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
@app.get("/admin/data-quality/status")
...
@app.get("/admin/data-quality/autofix-changes")
...
@app.get("/admin/data-quality/summary-by-province")
...
def admin_system_overview() -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
@app.get("/admin/system/self-test")
def admin_system_self_test() -> dict[str, Any]:
    if pipeline is None:
        raise RuntimeError("RAG pipeline is not initialized")
...
# --- /v1 aliases (same behavior as legacy paths; AI itinerary also mounted at /v1/ai/* above) ---
...
def v1_health_ready() -> JSONResponse:
    return health_ready()


@app.get("/v1/runtime/status", tags=["Runtime"])
def v1_runtime_status() -> dict[str, Any]:
    return runtime_status()


@app.post("/v1/rag/chat", tags=["RAG"])
def v1_rag_chat(request: RagChatRequest) -> dict[str, Any]:
    return rag_chat(request)


@app.post("/v1/rag/retrieve", tags=["RAG"])
def v1_rag_retrieve(request: RagRetrieveRequest) -> dict[str, Any]:
    return rag_retrieve(request)


@app.post("/v1/rag/chat/simple", tags=["RAG"])
def v1_rag_chat_simple(request: RagChatSimpleRequest) -> dict[str, Any]:
    return rag_chat_simple(request)